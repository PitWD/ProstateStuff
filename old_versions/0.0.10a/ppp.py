...
def LoadSettings(TrainType = 'Default'):
    ...
    iniVal = {
        ...
        'TriggerScrStart': config.get('Global', 'TriggerScrStart') if config.has_option('Global', 'TriggerScrStart') else 'termux-wake-lock',
        'TriggerScrStop': config.get('Global', 'TriggerScrStop') if config.has_option('Global', 'TriggerScrStop') else 'termux-wake-unlock',
    }
    ...
    if NoSettings:
        ...
        with open(config_file_path, 'w') as configfile:
            ...
            config.set('Global', 'TriggerScrStart', iniVal['TriggerScrStart'])
            config.set('Global', 'TriggerScrStop', iniVal['TriggerScrStop'])
    ...
    iniVal['TriggerScrStart'] = iniVal['TriggerScrStart'].strip('"')
    iniVal['TriggerScrStop'] = iniVal['TriggerScrStop'].strip('"')
...
def QuitApp(cls = 1):
    ...
    escCursorVisible(1)
    setEcho(1)
    # Check on Screensaver - command at end
    if iniVal['TriggerScreenSaver'] and iniVal['TriggerScrTime'] == 0:
        os.system(iniVal['TriggerScrStop'])
    if cls:
        escCLS()
    sys.exit(0)
...
# Check on ScreenSaver - command at start
if iniVal['TriggerScreenSaver'] and iniVal['TriggerScrTime'] == 0:
    os.system(iniVal['TriggerScrStart'])
